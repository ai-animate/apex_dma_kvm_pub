...
static void set_vars(uint64_t add_addr)
{
	...
	client_mem.Read<uint64_t>(add_addr + sizeof(uint64_t) * 102, smoothpred_addr);
	uint64_t smoothpred2_addr = 0;
	client_mem.Read<uint64_t>(add_addr + sizeof(uint64_t) * 103, smoothpred2_addr);
	// new weap nemesis
...
int main(int argc, char *argv[])
{
	...
	const char *cl_proc = "mmd_tool.exe";
